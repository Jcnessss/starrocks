...
struct MapAggAggregateFunctionState : public AggregateFunctionEmptyState {
    ...
    void update(MemPool* mem_pool,
                const KeyColumnType& arg_key_column,
                const Column& arg_value_column,
...
class MapAggAggregateFunction final
        : public AggregateFunctionBatchHelper<MapAggAggregateFunctionState<KT, MyHashMap>,
...
public:
    ...
    void reset(FunctionContext* ctx, const Columns& args, AggDataPtr state) const override {
        ...
        if (!state_impl.hash_map.empty()) {
            state_impl.hash_map.clear();
        }
    }
    ...
    void merge(FunctionContext* ctx, const Column* column, AggDataPtr __restrict state, size_t row_num) const override {
        ...
        if (offsets[row_num + 1] > offsets[row_num]) {
            this->data(state).update(ctx->mem_pool(),
