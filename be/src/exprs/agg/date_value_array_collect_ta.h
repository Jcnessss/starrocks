...
namespace starrocks {
struct DateValueArrayCollectState {

    void update(const DateColumn* date_column, const ArrayColumn* value_column, size_t row_num) {
        DateValue date = date_column->get_data()[row_num];
        dates.emplace_back(date.julian());
        auto array = value_column->get(row_num).get_array();
        std::vector<double> double_array;
        double_array.reserve(array.size());
        for (const auto& value : array) {
            double_array.emplace_back(value.get_double());
        }
        values[date.julian()] = double_array;
        if (values_num == 0) {
            values_num = array.size();
        }
    }

    void merge(int32_t date, std::vector<double> value) {
    ...
    phmap::flat_hash_map<int32_t, std::vector<double>> values;
};

class DateValueArrayCollectFunction final
        : public AggregateFunctionBatchHelper<DateValueArrayCollectState, DateValueArrayCollectFunction> {
public:
    ...
    void update(FunctionContext* ctx, const Column** columns, AggDataPtr __restrict state,
        ...
        auto value_column = down_cast<const ArrayColumn*>(ColumnHelper::get_data_column(columns[1]));
    ...
    void merge(FunctionContext* ctx, const Column* column, AggDataPtr __restrict state, size_t row_num) const override {
        ...
        size_t num;
        std::memcpy(&num, serialized.data + offset, sizeof(size_t));
        offset += sizeof(size_t);
        this->data(state).values_num = num;
        for (int i = 0; i < size; i++) {
            ...
            this->data(state).merge(date, value_array);
        }
    }
    ...
    void finalize_to_column(FunctionContext* ctx, ConstAggDataPtr __restrict state, Column* to) const override {
        ...
        bytes.resize(bytes.size() + 2 * sizeof(size_t) + size * (sizeof(int32_t) +
                 this->data(state).values_num * sizeof(double)));
        std::memcpy(bytes.data() + offset, &size, sizeof(size_t));
        offset += sizeof(size_t);
        std::memcpy(bytes.data() + offset, &this->data(state).values_num, sizeof(size_t));
        offset += sizeof(size_t);
        ...
        for (int i = 0; i < size; i++) {
            ...
            std::string s;
            s.append(std::to_string(v.julian()));
            for (int j = 0; j < this->data(state).values_num; j++) {
                double value = this->data(state).values.at(copy[i])[j];
    ...
    void convert_to_serialize_format(FunctionContext* ctx, const Columns& src, size_t chunk_size,
        ...
        const auto* value_column = down_cast<const ArrayColumn*>(ColumnHelper::get_data_column(src[1].get()));
        Bytes& bytes = dst_column->get_bytes();
        size_t offset = bytes.size();
        size_t value_num = value_column->offsets().get_data()[1] - 0;
        bytes.resize(bytes.size() + chunk_size * (2 * sizeof(size_t) + sizeof(int32_t) + value_num * (sizeof(double))));
        for (size_t i = 0; i < chunk_size; ++i) {
            ...
            std::memcpy(bytes.data() + offset, &size, sizeof(size_t));
            offset += sizeof(size_t);
            std::memcpy(bytes.data() + offset, &value_num, sizeof(size_t));
            offset += sizeof(size_t);
            if (size != 0) {
                std::memcpy(bytes.data() + offset, &date_column->get_data()[i], sizeof(int32_t));
                offset += sizeof(int32_t);
                auto array = value_column->get(i).get_array();
    ...
    void serialize_state(const DateValueArrayCollectState& state, BinaryColumn* dst) const {
        Bytes& bytes = dst->get_bytes();
        size_t offset = bytes.size();
        size_t size = state.dates.size();
        bytes.resize(bytes.size() + 2 * sizeof(size_t) + size * (sizeof(int32_t) + state.values_num * sizeof(double)));

        std::memcpy(bytes.data() + offset, &size, sizeof(size_t));
        offset += sizeof(size_t);
        std::memcpy(bytes.data() + offset, &state.values_num, sizeof(size_t));
        offset += sizeof(size_t);

        for (int i = 0; i < state.dates.size(); i++) {
            std::memcpy(bytes.data() + offset, &state.dates[i], sizeof(int32_t));
            offset += sizeof(int32_t);
            for (int j = 0; j < state.values_num; j++) {
                double value = state.values.at(state.dates[i])[j];
                std::memcpy(bytes.data() + offset, &value, sizeof(double));
                offset += sizeof(double);
            }
        }
    ...
    std::string get_name() const override { return "ta_date_value_array_collect"; }
};
