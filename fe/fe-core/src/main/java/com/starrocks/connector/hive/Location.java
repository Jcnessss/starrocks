...
public final class Location {
    ...
    public static Location of(String location) {
        ...
        if (location.startsWith("/")) {
            return new Location(location, Optional.empty(), Optional.empty(), Optional.empty(),
                    OptionalInt.empty(), location.substring(1));
        }

        List<String> schemeSplit = SCHEME_SPLITTER.splitToList(location);
        checkArgument(schemeSplit.size() == 2,
                "No scheme for file system location: %s", location);
        ...
        if (afterScheme.startsWith("//")) {
            ...
            Optional<String> userInfo = userInfoSplit.size() == 2 ?
                    Optional.of(userInfoSplit.get(0)) : Optional.empty();
            ...
            checkArgument((userInfo.isEmpty()
                            && host.isEmpty()
                            && port.isEmpty())
                    || authoritySplit.size() == 2,
                    "Path missing in file system location: %s", location);
        ...
        checkArgument(afterScheme.startsWith("/"),
                "Path must begin with a '/' when no authority is present");
        return new Location(location, Optional.of(scheme), Optional.empty(), Optional.empty(),
                OptionalInt.empty(), afterScheme.substring(1));
    }

    private Location(String location, Optional<String> scheme, Optional<String> userInfo, Optional<String> host,
                     OptionalInt port, String path) {
    ...
    public Location sibling(String name) {
        ...
        return this.withPath(location.substring(0, location.lastIndexOf('/') + 1) + name,
                path.substring(0, path.lastIndexOf('/') + 1) + name);
    }
    ...
    public Location parentDirectory() {
        ...
        verifyValidFileLocation();
        checkState(!path.isEmpty() && !path.equals("/"),
                "root location does not have parent: %s", location);
    ...
    public Location appendPath(String newPathElement) {
        ...
        checkArgument(!newPathElement.startsWith("/"),
                "newPathElement starts with a slash: %s", newPathElement);
     ...
     * Specifically, the path must not be empty and must not end with a slash.
     *
     * @throws IllegalStateException if the location is not a valid file location
     */
    public void verifyValidFileLocation() {
        ...
        checkState(!path.isEmpty() && !path.equals("/"),
                "File location must contain a path: %s", location);
    ...
    public boolean equals(Object o) {
        return (o instanceof Location) && location.equals(((Location) o).location);
    }
